...
print(f"--- Loaded API Key: {GEMINI_API_KEY[:7]} ---")
...
df = joblib.load(os.path.join(BASE_DIR, "embeddings_df.joblib"))


# def inference(prompt, model="deepseek-r1"):
...
def inference(prompt):
    ...
    # url = f"https://generativelanguage.googleapis.com/v1beta/models/{LLM_MODEL}:generateText"
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{LLM_MODEL}:generateContent"
    # Headers with Bearer token (not x-goog-api-key)
    headers = {
    ...
    # Correct JSON payload
      # Correct JSON payload for generateContent
    data = {
        "contents": [{
            "parts": [{
                "text": prompt
    ...
    try:
        ...
        r = requests.post(url, headers=headers, params=pa json=data)
...
# context_file = f"logs/context_{current_time}.json"
...
# prompt_file = f"logs/prompt_{current_time}.txt"
