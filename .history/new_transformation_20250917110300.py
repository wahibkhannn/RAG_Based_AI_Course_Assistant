...
# Paths
audio_folder = os.listdir("audios")
...
for audio in audio_folder:
    if audio.startswith(tuple([f"1.{i}" for i in range(1, 4)])) and audio.endswith(".mp3"):
        ...
        for segment in result["segments"]:
            ...
            output_path = os.path.join(output_folder, audio.replace(".mp3", ".txt"))
            with open(output_path, "w", encoding="utf-8") as f:
                 json.dump(chunks, f, ensure_ascii=False, indent=4)